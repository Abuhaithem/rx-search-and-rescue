...
/** Drop one pharmacy from the carrier's network — back to "no status on file". */
export async function removeCarrierPharmacyStatus(
  carrierId: string,
  planYear: number,
  pharmacyId: string,
  ...
  try {
    ...
    const input = z
      ...
      .parse({ carrierId, planYear, pharmacyId });
    ...
    const [removed] = await db
      ...
      .where(
        and(
          eq(carrierPharmacyNetworks.carrierId, input.carrierId),
          eq(carrierPharmacyNetworks.planYear, input.planYear),
          eq(carrierPharmacyNetworks.pharmacyId, input.pharmacyId),
        ),
      )
      .returning({ id: carrierPharmacyNetworks.id });
    if (!removed) return err("Network row not found");

    await writeAudit(db, {
      actorId: profile.id,
      action: "carrier.pharmacy_status_removed",
      entityType: "carrier",
      entityId: input.carrierId,
      meta: { pharmacyId: input.pharmacyId, planYear: input.planYear },
    });

    revalidatePath("/", "layout");
    return ok({ pharmacyId: input.pharmacyId });
  } catch (e) {
    return err(errorMessage(e));
  }
}
